...
/* using xmodmap and xev to figure out */
/*
...
#define TERMMOD (Mod1Mask|ShiftMask) /* shift + alt */

/*
 * using xev to figure out
 * normally mouse have 5 buttons:
 * left, right, wheel press, wheel forward, wheel backward
 */
...
static char *copyurlcmd[] = { "/bin/sh", "-c",
    "sed 's/.*│//g' | tr -d '\n' | grep -aEo '(((http|https)://|www\\.)[a-zA-Z0-9.]*[:]?[a-zA-Z0-9./&%?$#=_-]*)|((magnet:\\?xt=urn:btih:)[a-zA-Z0-9]*)' | uniq | sed 's/^www./http:\\/\\/www\\./g' | rofi -width 50 -dmenu -i -p 'Copy which url?' -l 10 | tr -d '\n' | xclip -selection clipboard", "externalpipe", NULL };
